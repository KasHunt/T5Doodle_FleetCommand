...
namespace TiltFive
{
    ...
    public sealed class Glasses : Singleton<Glasses>
    {
        ...
        /// <summary>
        /// Attempts to get the position and orientation of the specified player's glasses [In gameboard space].
        /// </summary>
        /// <param name="playerIndex"></param>
        /// <param name="pose"></param>
        /// <returns>Returns <c>false</c> along with an empty pose if something goes wrong.</returns>
        public static bool TryGetPhysicalPose(PlayerIndex playerIndex, out Pose pose)
        {
            if(Player.TryGetGlassesHandle(playerIndex, out var glassesHandle) && Instance.glassesCores.TryGetValue(glassesHandle, out var glassesCore))
            {
                pose = glassesCore.Pose_GameboardSpace;
                return true;
            }
            pose = new Pose();
            return false;
